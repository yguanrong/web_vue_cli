<template>
  <div class="video-view-p">
    <div class="title-class-p">
      <el-row>
        <el-col :span="8">
          <h3 class="l-text-p">{{label}}</h3>
        </el-col>
        <el-col :span="8">
          <h3 class="h-text-p">{{titleName}}</h3>
        </el-col>
        <el-col :span="8">
          <h3 class="r-text-p">{{time}}</h3>
        </el-col>
      </el-row>
      <hr class="line-p">
    </div>
    <video-comp ref="videoComponent" class="video-class-p"></video-comp>
    <h4 class="h4-num-p">集 数</h4>
    <div class="type-class-p">
      <el-button v-for="item in videoList" @click="changeVideo(item)" type="success">
        {{item.videoSub}}
      </el-button>
    </div>
    <div class="btn-class-p">
...
<script>
  import videoComp from '../../components/pc/VideoComp_p'
